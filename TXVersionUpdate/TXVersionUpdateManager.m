...
#import "TXVersionUpdateManager.h"
#import "TXAppStoreInfoModel.h"

@interface TXVersionUpdateManager ()
//本地info文件
@property (strong,nonatomic)NSDictionary *infoDict;
@end
@implementation TXVersionUpdateManager
...
+ (instancetype)shareManager {
    static TXVersionUpdateManager *instance;
...
+(void)checkNewEditionWithAppID:(NSString *)appID CustomAlert:(CheckVersionBlock)checkVersionBlock {
    [[self shareManager] getAppStoreVersion:appID sucess:^(TXAppStoreInfoModel * model) {
        if(checkVersionBlock)checkVersionBlock(model);
    }];
}

- (void)checkNewVersion:(NSString *)appID ctrl:(UIViewController *)containCtrl {
    [self getAppStoreVersion:appID sucess:^(TXAppStoreInfoModel *model) {
...
#pragma mark - 立即升级
- (void)updateRightNow:(TXAppStoreInfoModel *)model {
...
- (void)getAppStoreVersion:(NSString *)appID sucess:(void(^)(TXAppStoreInfoModel *))update {
    ...
    [self getAppStoreInfo:appID success:^(NSDictionary *respDict) {
        ...
        if (resultCount == 1) {
            ...
            //字典转模型
            TXAppStoreInfoModel *model = [[TXAppStoreInfoModel alloc] init];
